...
class BlockLayer(object):
    def __init__(self):
        ...
        self._block2_observe = {}


    def init_block2_option(self,transaction):
        #self._block1_sent = {}
        self._block2_sent = {}
        #self._block1_receive = {}
        self._block2_receive = {}
    ...
    def receive_request(self, transaction):
        ...
        if transaction.request.block2 is not None:
            ...
            key_token = hash(str(host) + str(port) + str(transaction.request.uri_path))
            ...
            if key_token in self._block2_receive:
                ...
                if num != 0:
                    self._block2_observe[key_token] = None
    ...
    def send_response(self, transaction):
        ...
        if (key_token in self._block2_receive and transaction.response.payload is not None) or \
            ...
            if transaction.request.observe == 1:
                m = 0
            if key_token in self._block2_observe:
                m = 0
                del self._block2_observe
    ...
    def send_request(self, request):
        ...
        if request.block1 or (request.payload is not None and len(request.payload) > defines.MAX_PAYLOAD):
            ...
            key_token = hash(str(host) + str(port) + str(request.uri_path))
            if request.block1:
